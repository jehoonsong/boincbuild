...
if grep -q '<soft_link>' myapp; then
    TEXT2=$(grep -oP '(?<=<soft_link>).*?(?=</soft_link>)' myapp) && cp $TEXT2 ./
fi
...
echo "run docker ..."
docker run -it --rm -v /tmp:/tmp -w $tmpr ghcr.io/nettargets/gromacs:gmx-2025.2-cuda-12.8 \
	bash run.sh 
