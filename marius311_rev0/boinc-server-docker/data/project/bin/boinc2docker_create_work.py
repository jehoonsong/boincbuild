    ...
    try:
        ...
        for layer in manifest[0]['Layers']:
            ...
            if force_reimport or (need_extract and not exists(layer_path)): 
                ...
                try:
                    # Ensure the directory exists before using tar
                    sh("mkdir -p %s" % dirname(layer_path_tar))
        ...
        # Check if the image path exists before using tar
        # if not exists(image_path_tar):
        #     import ipdb; ipdb.set_trace()
        #     raise IOError("Image tar file not found: {}".format(image_path_tar))
        ...
        if force_reimport or need_extract: 
            if verbose: 
                print fmt("Creating input file for image %s..."%image_id[:12])
            
            try:
                sh("tar cvf {image_path_tar} -C %s {image_id}.json manifest.json repositories"%tmpdir())
            except Exception as e:
                import ipdb; ipdb.set_trace()
                # print("Error occurred while creating tar: {}".format(e))
...
def get_image_id(image): 
    res = sh('docker inspect --format "{{ .Id }}" '+image).strip().split(':')[1]
    import ipdb; ipdb.set_trace()
    return res
